...
# Define the fitness function using Logistic Regression
def fitness_function(features):
    if not features:
        return 0.0

    clf = LogisticRegression(random_state=42, max_iter=5000)  # Increase max_iter for convergence
...
plt.title("PSO-like Feature Selection Progress for Logistic Regression (Classification)")
plt.xlabel("Iteration")
plt.ylabel("Best Accuracy")
plt.grid(True)
plt.show()

print('Selected Features (Logistic Regression - Classification):', selected_features_pso)
...
clf_selected_pso = LogisticRegression(random_state=42, max_iter=5000)
...
print('Accuracy with Selected Features (Logistic Regression - Classification):', accuracy_pso)
